...
async def main():
    print("= = = = = = = = = = = = = = = = = = = = = = = =")
    # data = Mongodb(database="屏科租屋網", collection="Houseinfo")

    # n = loop.run_until_complete(data.count({"rent": 4467}))
    """
    # 取得資料表內的資料數量
    n = data.count({"rent": 4467})
    pp.pprint(n)
    
    # 資料查找(返回全部)
    n = await data.find(query={"house_type": "分租套房"}, sort_key=["deposit", -1])
    pp.pprint(n)
    ...
    house = [
        {
            "picture": "../static/images/llimg_2.png",
            "Price": 26000.0,
            "Period": "七個月",
            "type": "套房",
            "ping": 10,
        ...
        {
            "picture": "../static/images/llimg_5.png",
            "Price": 45000.0,
            "Period": "二個月",
            "type": "套房",
            "ping": 9,
    ...
    """
    # 新增資料(可多筆，使用all=True)
    n = await data.insert(document=house)
    pp.pprint(f"新增的資料id: {n.inserted_id}")
    """

    """
    # 更新資料(可多筆，使用all=True)
    n = await data.update(filter={"ping": 10}, update={"$set": {"Period": "五個月"}})
    pp.pprint(f"更新了{n.modified_count}筆資料!")
    """

    """
    # 刪除資料(可多筆)
    n = await data.delete(filter={"Period": "七個月"})
